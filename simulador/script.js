...
(function () {
  ...
  const dom = {
    ...
    m3: $('input-m3'),
    angle: $('input-angle'),
    angleSlider: $('input-angle-slider'),
    muE: $('input-mu-e'),
    muESlider: $('input-mu-e-slider'),
    muK: $('input-mu-k'),
    muKSlider: $('input-mu-k-slider'),
    g: $('input-g'),
  ...
  function calc() {
    ...
    const f1_max = muE * N1,
      f2_max = muE * N2,
      fT_max = f1_max + f2_max;
    const Fnet = P3 - (P1x + P2x);

    let acc, T1, T2, dir, code, f1E, f2E, f1, f2;

    if (Math.abs(Fnet) <= fT_max) {
      // A força não superou o atrito estático → Equilíbrio (Aceleração Zero)
      dir = 'Equilíbrio';
      code = 'eq';
      acc = 0;
      f1E = (Fnet * m1) / (m1 + m2);
  ...
  function showTables(r) {
    ...
    const row = (rows) =>
      rows
        .map(
          (r) =>
            `<tr><td>${r[0]}</td><td><span class="formula">${r[1]}</span></td><td>${r[2]} N</td></tr>`
        )
        .join('');
  ...
  function showSteps(r) {
    ...
    if (r.code === 'eq')
      d6.push(
        `|F_net| ≤ f_max_total  →  <span class="hl">EQUILÍBRIO ESTÁTICO</span>`
      );
    else if (r.code === 'up')
      d6.push(
        `F_net > f_max_total  →  <span class="hl">Blocos SOBEM, m₃ DESCE</span>`
      );
    else
      d6.push(
        `|F_net| > f_max_total  →  <span class="hl">Blocos DESCEM, m₃ SOBE</span>`
      );
  ...
  function setBadge(r) {
    ...
    if (r.code === 'up') {
      ...
      b.textContent = '↘ Descem';
      b.classList.add('down');
    } else {
      b.textContent = '⚖ Equilíbrio';
      b.classList.add('eq');
    }
  }
  ...
  // ── Handlers ──
  function doCalc() {
    const vals = [dom.m1, dom.m2, dom.m3, dom.angle, dom.g].map((e) =>
      parseFloat(e.value)
    );
  ...
  function doClear() {
    ...
    dom.m3.value = 10;
    dom.angle.value = 30;
    dom.angleSlider.value = 30;
    dom.muE.value = 0.2;
    dom.muESlider.value = 20;
    dom.muK.value = 0.15;
    dom.muKSlider.value = 15;
    dom.g.value = 9.81;
    [dom.results, dom.tables, dom.steps].forEach((s) =>
  ...
  dom.muE.addEventListener('input', () => {
    dom.muESlider.value = Math.round(parseFloat(dom.muE.value) * 100);
    if (res) doCalc();
  });
  dom.muESlider.addEventListener('input', () => {
    dom.muE.value = (parseInt(dom.muESlider.value) / 100).toFixed(2);
    if (res) doCalc();
  });
  dom.muK.addEventListener('input', () => {
    dom.muKSlider.value = Math.round(parseFloat(dom.muK.value) * 100);
    if (res) doCalc();
  });
  dom.muKSlider.addEventListener('input', () => {
    dom.muK.value = (parseInt(dom.muKSlider.value) / 100).toFixed(2);
    if (res) doCalc();
  });
  [dom.m1, dom.m2, dom.m3, dom.g].forEach((e) =>
